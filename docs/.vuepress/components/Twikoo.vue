...
<script>
const waitTime = 700 // 页面加载后多少毫秒后加载评论区（如果是 0ms，可能会报错）
export default {
  data() {
    return {
      twikoo: '',
  ...
  methods: {
    twikooInit() {
      twikoo.init({
        ...
        envId: 'https://twikoo-ten-eta.vercel.app/',
        el: '#tcomment',
    ...
    // 跳转新页面，重新获取当前页面的评论信息
    updateComment() {
      const tk_icon = document.getElementsByClassName('tk-icon')[0]
    ...
    // 其他页面
    otherPage(route) {
      return route.path === '/archives/' || route.path === '/categories/' || route.path === '/tags/'
    }
  }
}
</script>
