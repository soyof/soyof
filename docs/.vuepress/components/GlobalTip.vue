...
export default {
  watch: {
    $route(to, from) {
      const gloablTip = document.getElementsByClassName('gloablTip')
...
function addTip(content, type, startHeight = 50, dieTime = 3000) {
  const tip = document.querySelectorAll('.global-tip')
  const time = new Date().getTime()
  // 获取最后消息提示元素的高度
  const top = tip.length == 0 ? 0 : tip[tip.length - 1].getAttribute('data-top')
  ...
  const timeTip = document.getElementsByClassName(time)[0]
  setTimeout(() => {
  ...
  setTimeout(() => {
    ...
    // 下面的所有元素回到各自曾经的出发点
    const allTipElement = nextAllTipElement(timeTip)
    for (let i = 0; i < allTipElement.length; i++) {
      const next = allTipElement[i]
      const top =
